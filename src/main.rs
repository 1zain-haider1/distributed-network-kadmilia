...
async fn main() {
	...
	let ip = utils::get_local_ip().unwrap();
	let port = 1338;
	let node0 = Node::new(ip, port);
	println!("[+] Created Node Zero: {:?}", node0);
	...
	let mut handles = vec![];
	// server::start_server(node0).await;
	 // start the actix server
	 handles.push(tokio::spawn( async move {
        server::start_server(node0.clone()).await.unwrap()
    }));
